# Madia Consultoria Digital — v2 (Design)
...
Segunda versão do site da Madia Consultoria Digital, refeita do zero: novo design (escuro e sofisticado), novo posicionamento (marca pessoal — Lucca à frente) e nova base técnica. O design segue fielmente um prompt de referência de landing page de portfólio ("Jack — 3D Creator", Apêndice A), adaptando conteúdo, idioma e seções para a Madia.

## Decisões de produto

| Tema | Decisão |
|---|---|
| Posicionamento | Marca pessoal (Lucca), tom "eu" |
| Serviços | Os mesmos 5 da v1 |
| Idioma | Somente português (sem i18n) |
| Contato | WhatsApp direto, sem backend/formulário |
| Estilo visual | Escuro (#0C0C0C) e sofisticado, fiel ao prompt de referência |
| Hero | Palavra única gigante: **MADIA** |
| Imagens | URLs externas do prompt como placeholder; troca futura via `content.ts` |
| Projetos e depoimentos | Conteúdo fictício/placeholder por ora |
| Localização do código | `/root/consultoria-site-v2` (WSL) + atalho na pasta Windows "00. Madia - Consultoria Digital v2" |
...
- Framer Motion ^12
- Lucide React
- Fonte: Kanit (Google Fonts, pesos 300–900)
- Sem testes unitários; verificação visual via `npm run dev` (desktop e mobile)
- Deploy futuro: Vercel (fora do escopo desta spec)

## Estrutura do projeto

```
src/
  components/ui/        FadeIn, Magnet, AnimatedText, ContactButton, LiveProjectButton
  components/sections/  HeroSection, MarqueeSection, AboutSection, ServicesSection,
                        ProjectsSection, DepoimentosSection, FormacoesSection, ContatoSection
  content.ts            todos os textos, links e URLs de imagem centralizados
```

`content.ts` é o único lugar com copy, links e imagens — trocar placeholders por conteúdo real não deve exigir mexer em componentes.

## Seções (em ordem)

As seções 1–5 seguem o Apêndice A à risca (layout, medidas, animações, breakpoints), com as adaptações de conteúdo abaixo. As seções 6–8 são novas, no mesmo estilo visual.

### 1. Hero
- Navbar: **Sobre, Serviços, Projetos, Contato** (âncoras para as seções).
- H1 gigante: **"MADIA"** com o gradiente `.hero-heading`.
- Texto à esquerda (bottom bar): "soluções administrativas, tecnológicas e visuais para o seu negócio" (uppercase, light).
- ContactButton à direita: **"Fale comigo"** → link WhatsApp (abaixo).
- Retrato central com efeito Magnet: imagem do Jack (placeholder; trocar pela foto do Lucca).
- Título da aba: **"Madia — Consultoria Digital"**.

### 2. Marquee
Igual ao Apêndice A: duas fileiras de GIFs (21 URLs de motionsites.ai), movidas pelo scroll — fileira 1 para a direita, fileira 2 para a esquerda.

### 3. Sobre mim (About)
- Heading gradiente: **"SOBRE MIM"**.
- Ícones 3D decorativos nos 4 cantos (URLs do prompt).
- Parágrafo com revelação letra por letra no scroll (AnimatedText). Texto: "Com mais de cinco anos de experiência entre administração e tecnologia, ajudo negócios a crescer com processos organizados, dados claros e presença digital marcante. Vamos construir algo incrível juntos!"
- ContactButton abaixo do texto.

### 4. Serviços (Services)
Fundo branco, cantos superiores arredondados, heading **"SERVIÇOS"**, lista numerada 01–05 com textos da v1:

1. **Consultoria Administrativa** — Otimização de processos e estruturas para máxima eficiência e crescimento sustentável.
2. **Dashboards Interativos** — Visualização de dados em tempo real para decisões mais rápidas e embasadas.
3. **Sites e Sistemas** — Desenvolvimento de soluções digitais sob medida, do site institucional ao sistema completo.
4. **Social Media** — Gestão estratégica de perfis digitais para ampliar presença e engajamento online.
5. **Design Gráfico** — Identidade visual e materiais que comunicam autoridade e profissionalismo.

### 5. Projetos (Projects)
- Heading gradiente: **"PROJETOS"**.
- 3 cards sticky-stacking (escala 1 − (total−1−i)·0.03, offset 28px/card), borda #D7E2EA, exatamente como o Apêndice A.
- Conteúdo placeholder do prompt: Nextlevel Studio (Cliente), Aura Brand Identity (Pessoal), Solaris Digital (Cliente), com as imagens CloudFront originais. Botão "Live Project" vira **"Ver projeto"** (sem link real por ora — href "#").
- Categorias traduzidas: Client → Cliente, Personal → Pessoal.

### 6. Depoimentos (nova)
- Fundo escuro #0C0C0C, heading gradiente **"DEPOIMENTOS"** no mesmo estilo dos demais.
- 3 cards placeholder: citação, nome, empresa. Bordas sutis (rgba de #D7E2EA), cantos bem arredondados, grid 3 colunas no desktop / empilhado no mobile.
- Conteúdo placeholder (fictício, trocar depois em `content.ts`):
  1. "O dashboard mudou a forma como enxergamos o negócio. Decisões que levavam dias agora saem na hora." — Ana Souza, Comércio local
  2. "Site entregue rápido, bonito e do jeito que pedimos. Recomendo de olhos fechados." — Carlos Lima, Prestador de serviços
  3. "Organização dos processos que a gente nem sabia que precisava. Profissional e atencioso do início ao fim." — Marina Alves, Clínica
- Entrada com FadeIn em stagger (i · 0.1), como a lista de serviços.

### 7. Formações (nova)
- Fundo escuro, heading gradiente **"FORMAÇÕES"**.
- Duas colunas no desktop (empilhadas no mobile): **Formação Acadêmica** e **Certificações & Cursos**, com separadores de 1px no estilo da lista de serviços (versão clara: rgba(215, 226, 234, 0.15)).
- Dados reais da v1:
  - Acadêmica: Bacharelado em Administração (cursando) — AEMS, Três Lagoas — conclusão 2028.
  - Certificações: Pós-Graduação Power BI Avançado (Ipog, 2026); Curso de Extensão — Cronograma e Ferramentas de Produtividade (Ipog, 2026); Programação Python (Curso em Vídeo — Gustavo Guanabara, 2026).

### 8. Contato / Footer (nova)
- Fechamento de página: heading gigante gradiente **"VAMOS CONVERSAR?"**.
- ContactButton grande: **"Chamar no WhatsApp"** → `https://wa.me/+5567998746300?text=Ol%C3%A1%2C%20vim%20pelo%20seu%20site%2C%20fiquei%20interessado(a)%20no%20seu%20trabalho.`
- Footer: e-mail **madiaconsultoria@gmail.com** e "© Madia Consultoria Digital — Todos os direitos reservados".

## Componentes reutilizáveis

Conforme Apêndice A: **ContactButton** (pílula com gradiente roxo/laranja e outline branco), **LiveProjectButton** (ghost, borda #D7E2EA), **FadeIn** (whileInView, once, easing [0.25, 0.1, 0.25, 1]), **Magnet** (efeito magnético no retrato), **AnimatedText** (opacidade por caractere via useScroll, offset ['start 0.8', 'end 0.2']).

Todos os links de contato do site (navbar "Contato" desce até a seção 8; botões "Fale comigo"/"Chamar no WhatsApp" abrem o WhatsApp em nova aba).

## Fora do escopo (v2.1+)

- Foto real do Lucca, projetos reais, depoimentos reais (troca via `content.ts`)
- Deploy na Vercel e domínio
- Formulário de contato, blog, i18n

---

## Apêndice A — Prompt de referência (verbatim)

> O texto abaixo é a referência visual/técnica original em inglês. Onde conflitar com as seções acima (idioma, textos, seções extras, labels), **as seções acima prevalecem**.
...
Heading: same gradient/style as other headings.

3 sticky-stacking project cards that scale down as you scroll past them (Framer Motion useScroll and useTransform). Each card is sticky top-24 md:top-32 inside an h-[85vh] container. Scale: targetScale = 1 - (totalCards - 1 - index) * 0.03. Each card offset by top: ${index * 28}px.
...
Card layout:
- Top row: Number (huge, same style as services), category label, project name, and a ghost "Live Project" button (rounded-full, border-2 #D7E2EA, uppercase, tracking-widest).
- Bottom row: Two-column image grid -- left column (40% width) has 2 stacked images, right column (60%) has 1 tall image. All images rounded-[40px] sm:rounded-[50px] md:rounded-[60px]. Left top image height: clamp(130px, 16vw, 230px). Left bottom image height: clamp(160px, 22vw, 340px).
